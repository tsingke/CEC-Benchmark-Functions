function [gbest,gbestval,fitcount,ss]= CPSO_func(fhd,Max_Gen,Max_FES,Particle_Number,Dimension,VRmin,VRmax,func)
...
vel1=zeros(ps,D);
%pos(1,:)=[ 4.6423   -5.0000    3.6601    3.5399    4.6814    3.7970    1.9219   -4.7207    0.1088   -1.2431];
%pos1(1,:)=[4.6423   -5.0000    3.6601    3.5399    4.6814    3.7970    1.9219   -4.7207    0.1088   -1.2431];

for k=1:20
     for j=1:D
                 pos(k,j)=memory1(ceil(rand*20),j);
               %  pos1(k,j)=memory1(ceil(rand*20),j);
     end
end
pos1=pos;
%pos(1,:)=vector_cpsomhs';
%pos1(1,:)=vector_cpsomhs';
for j=1:ps
    e(j,1)=feval(fhd,pos(j,:)',func);
    e1(j,1)=feval(fhd,pos1(j,:)',func);
end
...
%     if (sum(pos1(k,group_d(j,:))>VRmax(k,group_d(j,:)))+sum(pos1(k,group_d(j,:))<VRmin(k,group_d(j,:))))==0;
    e1(k,j)=feval(fhd,pos1_temp',func);
%     end
    fitcount=fitcount+1;
